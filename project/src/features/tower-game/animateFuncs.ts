...
import { checkMoveDown, getMoveDownValue, drawYellowString, getAngleBase } from './utils';
import { addFlight } from './flight';
import * as constant from './constant';

export const endAnimate = (engine: Engine) => {
  const gameStartNow = engine.getVariable(constant.gameStartNow);
  if (!gameStartNow) return;
  const successCount = (engine.getVariable(constant.successCount, 0) as number) || 0;
  const failedCount = engine.getVariable(constant.failedCount) as number;
  const gameScore = (engine.getVariable(constant.gameScore, 0) as number) || 0;
...
export const startAnimate = (engine: Engine) => {
  const gameStartNow = engine.getVariable(constant.gameStartNow);
  if (!gameStartNow) return;
  ...
  if (!lastBlock || [constant.land, constant.out].indexOf(lastBlock.status) > -1) {
    ...
    const random = engine.utils.random as (a: number, b: number) => number;
    const randomPositiveNegative = engine.utils.randomPositiveNegative as () => number;
    const initialAngle =
      (Math.PI * random(angleBase, angleBase + 5) * randomPositiveNegative()) / 180;
    engine.setVariable(constant.blockCount, (engine.getVariable(constant.blockCount) as number) + 1);
    engine.setVariable(constant.initialAngle, initialAngle);
    engine.setTimeMovement(constant.hookDownMovement, 500);
    const block = new CoolInstance({
