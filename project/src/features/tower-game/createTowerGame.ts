...
import { Engine as CoolEngine, Instance } from 'cooljs';
import { touchEventHandler } from './utils';
import type { TowerGameUserOption } from './utils';
...
export type CreateTowerGameOptions = TowerGameUserOption & {
  /** Базовый URL ассетов, например `/tower-game/assets/` */
  assetsBaseUrl?: string;
};

function patchEngineDispose(game: Engine) {
  const originalAnimate = game.animate.bind(game);
  game.animate = (time: number) => {
    if (game.__towerDisposed) return;
    originalAnimate(time);
  };
}

export function createTowerGame(option: CreateTowerGameOptions): Engine {
  const {
    width,
    height,
    canvasId,
  ...
  const pathGenerator = (path: string) =>
    `${assetsBaseUrl.replace(/\/?$/, '/')}${path.replace(/^\//, '')}`;

  const game = new CoolEngine({
    canvasId,
    highResolution: true,
    width,
    height,
  ...
  game.setVariable(
    ...
    (game.getVariable(constant.blockWidth) as number) * 0.71
  );
