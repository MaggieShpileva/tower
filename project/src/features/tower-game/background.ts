...
export const backgroundImg = (engine: Engine) => {
  const bg = engine.getImg('background');
  const bgWidth = bg.width;
  const bgHeight = bg.height;
  const zoomedHeight = (bgHeight * engine.width) / bgWidth;
  let offsetHeight = engine.getVariable(constant.bgImgOffset, engine.height - zoomedHeight) as number;
  ...
  engine.getTimeMovement(
    ...
    [
      [
        offsetHeight,
        offsetHeight + getMoveDownValue(engine, { pixelsPerFrame: (s) => s / 2 }),
      ],
    ],
    (value: number) => {
      offsetHeight = value;
...
const getLinearGradientColorRgb = (
  colorArr: number[][],
  colorIndex: number,
  proportion: number,
) => {
  ...
  const calRgbValue = (index: number) => {
...
export const backgroundLinearGradient = (engine: Engine) => {
  ...
  const offsetHeight = engine.getVariable(constant.bgLinearGradientOffset, 0) as number;
...
export const background = (engine: Engine) => {
