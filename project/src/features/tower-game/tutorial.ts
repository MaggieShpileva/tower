...
export const tutorialAction = (instance: Instance, engine: Engine, time: number) => {
...
export const tutorialPainter = (instance: Instance, engine: Engine) => {
  ...
  const { ctx } = engine;
  const { name } = instance;
  const t = engine.getImg(name as string);
